...
class MemoryStore(ABC):
    ...
    def fetch_from_source(self, tenant_id: str, session_id: str, store_id: str = "") -> List[ChatMessage]:
        ...
        携带身份头防止越权，追加 ?limit=N 参数按时间倒序取最近 N 条
        ...
        try:
            ...
            params = {"limit": storage_settings.JAVA_SESSION_MESSAGES_LIMIT}
            result = java_http_client.get_sync(
                ctx,
                storage_settings.JAVA_SESSION_MESSAGES_PATH.format(session_id=session_id),
                params=params,
            )
...
class RedisMemoryStore(MemoryStore):
    ...
    def __init__(self):
        # 复用全局 Redis 单例 (惰性连接 + ping 探测, Redis 不可用时返回 None, 调用方降级内存)

...
import 调整：
- 包内依赖 store/window 改为 new_agent.memory.store / new_agent.memory.window；
- 观测埋点从旧 obs.metrics（metrics）统一为 core.obs.metrics（otel_metrics），
...
from new_agent.memory.store import MemoryStore, memory_store
from new_agent.memory.window import memory_window, MessageCompressor

...
def _chunk_key(doc: LCDocument) -> str:
    """生成 chunk 唯一标识: 优先 metadata.chunk_id, 回退 doc_id+内容前缀, 再回退内容哈希.

    用于跨多路召回去重 (同一 chunk 在向量 + BM25 两路命中时只保留一份, 分数累加).
    D1 决策 4 存储优化后, 向量库 page_content 仅存 chunk_id 占位符 (非全文),
    故必须优先用 metadata.chunk_id 去重, 否则向量路 (占位符) 与 BM25 路 (全文)
    的同一 chunk 因 page_content 不同而无法去重.
    """
    ...
    for docs, source in multi_results:
        for rank, doc in enumerate(docs):
            ...
            if key in fused:
                ...
                meta = doc.metadata or {}
                chunk = DocumentChunk(
                    # D1 决策 6: chunk_id 优先取 metadata (稳定标识 {doc_id}_{idx}),
                    # 回退 doc_id (兼容未走决策 6 的旧 chunk)
                    chunk_id=str(meta.get("chunk_id", "")) or str(meta.get("doc_id", f"chunk-{rank}")),

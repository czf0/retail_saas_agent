...
class MemoryStore(ABC):
    ...
    def fetch_from_source(self, tenant_id: str, session_id: str, store_id: str = "") -> List[ChatMessage]:
        ...
        携带 X-Tenant-ID / X-Store-ID 头防止越权，返回最近 20 条消息（时间正序）。
        """
        import httpx
        base_url = storage_settings.JAVA_BACKEND_BASE_URL
        url = f"{base_url}/api/v1/chat/internal/sessions/{session_id}/messages"
...
class RedisMemoryStore(MemoryStore):
    ...
    def _key(self, tenant_id: str, session_id: str) -> str:
        # 多租户隔离：key 前缀携带租户 ID
        return f"agent_memory:{tenant_id or 'default'}:{session_id}"
    ...
    def _lock_key(self, tenant_id: str, session_id: str) -> str:
        return f"agent:tenant:{tenant_id or 'default'}:session:{session_id}:lock"

...
class AuditStore:
    ...
    def write(self, record: dict) -> None:
        ...
        try:
            ...
            # 审计写入失败不阻断业务, 仅告警; 并按铁律 17 增计可观测指标, 便于运维感知落盘故障
            logger.warning(f"audit_write_failed err={exc}")
            otel_metrics.incr("audit_write_failed")
    ...
    def query(
        ...
        limit: int = observability_settings.AUDIT_QUERY_DEFAULT_LIMIT,
    ) -> List[dict]:
        ...
        if date_from and date_to:
            try:
                ...
                dt_from = datetime.now() - timedelta(days=observability_settings.AUDIT_QUERY_DEFAULT_DAYS)
                dt_to = datetime.now()
        else:
            dt_from = datetime.now() - timedelta(days=observability_settings.AUDIT_QUERY_DEFAULT_DAYS)
            dt_to = datetime.now()

...
from unified_agent.hitl_state import (
    clear_pending_thread,
    get_pending_thread,
    save_pending_thread,
)
from unified_agent.llm import unified_llm_client
from unified_agent.memory import build_checkpointer
from unified_agent.obs.metrics import otel_metrics
from unified_agent.obs.tracer import otel_tracer, traced
from unified_agent.tool import load_langchain_tools

from unified_agent.intent_router import resolve_intent, detect_scenario
from unified_agent.obs.token_accumulator import (
    TokenAccumulatorHandler,
    get_token_total,
    reset_token_total,
)
from unified_agent.prompt import get_provider, build_tool_shortlist_prompt, PROMPT_VERSION
from unified_agent.state import UnifiedState
...
class UnifiedGraph:
    ...
    async def _react_execute_node(self, state: UnifiedState) -> dict:
        ...
        try:
            from unified_agent.prompt import PROMPT_VERSION

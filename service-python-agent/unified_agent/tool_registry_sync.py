...
# ============================================================================
# 请求头构造 (与 JavaBackendTool._build_headers 对齐, 透传身份/链路标识)
# ============================================================================

def _java_base_url() -> str:
    """Java 后端基础地址 (复用 storage_settings, 与 memory_store 回源拉取一致)."""
    return storage_settings.JAVA_BACKEND_BASE_URL


def _build_headers() -> Dict[str, str]:
    """构造 Java 调用请求头, 透传身份/链路标识.

    与 JavaBackendTool._build_headers 对齐: Java GlobalReqInterceptor 校验
    X-Internal-Secret 后建立临时登录态, 使 @SaCheckPermission 基于 X-User-ID 校验.
    """
...
def fetch_registry_sync() -> List[Dict[str, Any]]:
    ...
    """
    url = f"{_java_base_url()}{agent_flow_settings.JAVA_TOOL_REGISTRY_PATH}"
    headers = _build_headers()
    try:
        with httpx.Client(timeout=agent_flow_settings.JAVA_TOOL_TIMEOUT) as client:
            resp = client.get(url, headers=headers)
            resp.raise_for_status()
            items = _parse_registry_response(resp.json())
...
async def fetch_registry_async() -> List[Dict[str, Any]]:
    ...
    headers = _build_headers()
    try:
        async with httpx.AsyncClient(timeout=agent_flow_settings.JAVA_TOOL_TIMEOUT) as client:

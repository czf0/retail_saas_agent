...
# Redis 客户端单例 (与 memory_store / RedisSaver 共享同一 Redis 实例, 不同 key 前缀)
_redis_client = None
...
def _get_redis():
    """获取 Redis 客户端单例 (懒加载, 线程安全).

    与 memory_store / RedisSaver 共享同一 Redis 实例配置 (host/port/db/password),
    但使用独立的 key 前缀 (hitl_pending:), 互不干扰.
    Redis 不可用时返回 None, 调用方降级为进程内存.
    """
    ...
    with _redis_lock:
        ...
        try:
            ...
            _redis_client.ping()
            logger.info("HITL pending store Redis 连接成功")
        except Exception as exc:
            logger.warning(f"HITL pending store Redis 不可用, 降级内存存储: {exc}")
            _redis_client = None
    return _redis_client

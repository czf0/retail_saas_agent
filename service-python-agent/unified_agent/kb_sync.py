...
迁移说明 (剔除 streamchat 流程适配代码):
- 本模块从 flow_architecture/kb_sync.py 复制迁入 unified_agent 包, 源文件保留不删;
- import 由 other_agent.obs / other_agent.rag 统一切换为 unified_agent.obs / unified_agent.rag,
  切断对 other_agent 包的依赖 (根治 OTel 双实例初始化冲突: 原链路 main → kb_sync_router
  → flow_architecture.kb_sync → other_agent.obs 触发第二次 init_otel);
- RAG 引擎由 lc_rag_engine 替换为 unified_rag_engine (接口完全兼容: ingest/delete 签名一致);
- 路由层 api/kb_sync_router.py 的 import 同步指向本模块.
...
from core.constants import DEFAULT_ROLE_ID
from core.logger import get_logger
from unified_agent.obs.metrics import otel_metrics
from unified_agent.obs.tracer import otel_tracer
from unified_agent.rag.rag_cache import rag_cache
from unified_agent.rag.rag_engine import unified_rag_engine
from unified_agent.rag.synonym_client import invalidate_local_cache as invalidate_synonym_cache
from schema.rag_schema import Document

logger = get_logger("unified_kb_sync")
...
class KbSyncResult:
    ...
    D1.2 扩展: chunks 字段携带 ingest 产生的分片元信息 (List[ChunkInfo 序列化 dict]),
    由 Java KnowledgeSyncNotifier 解析后落库 kb_doc_chunk, 供管理员查看分片明细.
    仅 doc_upsert/full_rebuild 事件填充, 其他事件为空列表.
    """

    def __init__(self, ok: bool, message: str = "", affected: int = 0, chunks: list = None):
        # 是否处理成功 (业务级, 非系统异常)
        self.ok = ok
        # 结果描述 (供 Java 侧日志与排障)
        ...
        # D1.2: 分片元信息 (chunk_id/chunk_index/content_head/content_tail/char_count/chunk_type),
        # 供 Java 落库 kb_doc_chunk; 非入库事件为空列表
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "affected": self.affected, "chunks": self.chunks,
        }
...
async def _handle_doc_upsert(
    ...
    payload 结构:
        {"docs": [{"doc_id": "...", "title": "...", "content": "...",
                    "domain": "promo", "role_id": "",
                    "store_id": "", "valid_until": "2026-12-31",
                    "version": 1, "metadata": {}}]}

    业务过滤字段 (domain/role_id/store_id/valid_until) 透传到向量库 metadata,
    ...
    # 若 Java 推送混合域文档, 每个文档的 metadata 内仍带自己的 domain/role_id,
    ...
    for i, item in enumerate(docs_raw):
        ...
        if item.get("role_id"):
            meta.setdefault("role_id", item["role_id"])
        ...
        if i == 0:
            ...
            role_id = item.get("role_id", DEFAULT_ROLE_ID) or DEFAULT_ROLE_ID
    ...
    # D1.2: ingest 返回 List[ChunkInfo] (分片元信息), 回传 Java 落库 kb_doc_chunk
    chunk_infos = await unified_rag_engine.ingest(
        tenant_id=tenant_id,
        documents=documents,
    ...
    # ChunkInfo → dict 序列化 (Pydantic model_dump, 供 JSON 响应回传 Java)
    chunks_payload = [c.model_dump() for c in chunk_infos] if chunk_infos else []
    return KbSyncResult(
        ok=True,
        message=f"upserted docs={len(documents)} chunks={len(chunks_payload)}",
...
async def _handle_doc_delete(
    ...
    for doc_id in doc_ids:
        ...
        if not doc_id:
            continue
        # delete 内部已失效检索缓存
        await unified_rag_engine.delete(tenant_id=tenant_id, doc_id=doc_id)
        affected += 1
    return KbSyncResult(
        ok=True,

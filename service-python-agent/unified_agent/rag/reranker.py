...
from unified_agent.rag.tokenizer import chinese_tokenize
...
class HttpReranker:
    ...
    def __init__(self) -> None:
        # 复用 rag_settings 的 rerank 服务地址; 超时取统一 HTTP 客户端超时配置
        self._url = rag_settings.RAG_RERANK_MODEL_URL
        self._timeout = float(agent_flow_settings.HTTP_CLIENT_TIMEOUT)
...
class LocalScoreReranker:
    ...
    async def rerank(self, query: str, fused: List[FusedResult], top_k: int) -> List[RerankResult]:
        ...
        for f in fused:
            ...
            # 加权评分: RRF 分 + 词命中率, 权重取自 rag_settings (二者和建议为 1.0)
            rrf_weight = rag_settings.RERANK_LOCAL_RRF_WEIGHT
            hit_weight = rag_settings.RERANK_LOCAL_HIT_WEIGHT
            score = rrf_weight * rrf_norm + hit_weight * hit_rate

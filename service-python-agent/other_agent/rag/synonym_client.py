...
# Redis 读 ~1ms, 进程内 ~0; 30s TTL 平衡实时性与性能.
_LOCAL_CACHE: Dict[tuple, tuple] = {}
_LOCAL_TTL = 30.0

# 模块级 Redis 单例 (惰性构建, 失败后不重试避免每次检索都尝试连接)
_redis_client = None
_redis_inited = False


def _build_redis():
    """惰性构建 Redis 客户端, 复用 storage_settings 连接参数.

    不可用时返回 None (降级), 与 memory_store 降级策略一致.
    """
    try:
        import redis
        client = redis.Redis(
        ...
        logger.warning(f"synonym_redis_unavailable: {exc}")
        return None
...
def _read_from_redis(tenant_id: str, domain: str) -> Dict[str, List[str]]:
    ...
    try:
        ...
        raw_global = client.get("kb:synonym:global")
        if raw_global:
            result.update(json.loads(raw_global))
        # 租户+域特定同义词 (scope=tenant), 与 global 取并集去重
        key = f"kb:synonym:{tenant_id or 'default'}:{domain or 'all'}"

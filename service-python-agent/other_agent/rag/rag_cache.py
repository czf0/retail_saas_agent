...
- key = (tenant_id, role_scope, domain, canonical_query) 四维隔离
...
使 `from other_agent.rag.rag_cache import rag_cache` 可直接拿到单例调用 .get/.put.
...
class RagCache:
    ...
    @staticmethod
    def _make_key(tenant_id: str, role_scope: str, domain: str, canonical_query: str) -> str:
        """构造缓存 key: 四维隔离 (租户+角色+域+query 哈希).

        query 取 MD5 哈希避免长 query 作 key 的内存开销.
        """
        raw = f"{tenant_id or 'default'}|{role_scope or 'all'}|{domain or 'all'}|{canonical_query or ''}"
        return hashlib.md5(raw.encode("utf-8")).hexdigest()

    def get(self, tenant_id: str, role_scope: str, domain: str, canonical_query: str) -> Optional[RagContext]:
        """查询缓存, 命中且未过期则返回 RagContext, 否则 None."""
        if not canonical_query:
            return None
        key = self._make_key(tenant_id, role_scope, domain, canonical_query)
    ...
    def put(self, tenant_id: str, role_scope: str, domain: str, canonical_query: str, ctx: RagContext) -> None:

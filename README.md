...
- 📋 **向量库适配**：当前用的Chroma轻量级向量库，后续知识文档数量膨胀可以考虑换成其他向量库。
- 📋 **并发问题**：
        1.当前是Java直连Python，后续考虑是否需要引入消息队列异步调用Python->SSE又该怎么实现。
        2.引入线程池、限流机制、熔断机制。
        3.多Agent实例，分摊并发压力。
        ...
- 📋 **交互优化**：
        1.展示部分Agent节点操作，提升用户主观上的响应时间，提升交互体验。
        2.当前的人工介入提示略微粗糙，后续考虑优化，最好可以展示执行前后数据DIFF。
        ...
- 📋 **多租户 / 角色差异化预算(待定)**：按 `tenant_id` / `role` 分级限流。
- 📋 **长期记忆升级（待定）**：显式记忆抽取（摘要/实体）+ 按用户维度向量记忆。
